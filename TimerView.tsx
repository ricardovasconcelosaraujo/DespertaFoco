...
export const TimerView: React.FC<TimerViewProps> = ({ playAudio, stopAudio, previewSound }) => {
  ...
  useEffect(() => {
    if (isRunning && timeLeft > 0) {
      document.title = `⏲️ ${formatDisplayTime(timeLeft)} | Temporizador`;
    } else if (isRinging) {
      document.title = '🚨 TEMPO ESGOTADO!';
    }
  }, [timeLeft, isRunning, isRinging]);
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in zoom-in-95 duration-500 py-4 relative">
      <div className="relative mb-8">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          {isRunning && <div className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-2 animate-pulse">Contando...</div>}
        </div>
      </div>

      {!isRinging && (
        <div className="flex gap-6 items-center mb-10">
          <button onClick={isRunning ? pauseTimer : startTimer} className={`h-20 w-20 rounded-3xl flex items-center justify-center transition-all shadow-xl hover:scale-105 active:scale-95 ${isRunning ? 'bg-white dark:bg-slate-800 text-amber-600 dark:text-amber-400 border-2 border-amber-500/30' : 'bg-blue-600 text-white shadow-blue-600/30 hover:bg-blue-500'}`} disabled={timeLeft === 0 && !isRunning}>
            {isRunning ? <Pause size={32} fill="currentColor" /> : <Play size={32} fill="currentColor" className="ml-1" />}
          </button>
          <button onClick={resetTimer} className="p-5 rounded-3xl bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700 shadow-lg active:rotate-180 duration-500" title="Reiniciar">
            <RefreshCw size={26} />
          </button>
        </div>
      )}
          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            {[
              ...
              <div key={item.label} className="flex flex-col text-center">
                <label className="text-[10px] font-bold text-slate-400 uppercase mb-2">{item.label}</label>
                <select value={item.value} onChange={(e) => item.setter(Number(e.target.value))} className="w-full appearance-none bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-2 py-3 text-2xl font-mono font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer transition-all text-center">
                  {item.options.map(val => <option key={val} value={val}>{String(val).padStart(2, '0')}</option>)}
                </select>
              </div>
            ))}
          </div>
          <button onClick={clearSelectors} className="w-full py-2 mb-6 text-[10px] font-bold text-slate-400 hover:text-red-500 flex items-center justify-center gap-2 transition-colors uppercase tracking-widest"><Trash2 size={12} /> Zerar Tempos</button>
          <div className="space-y-2 pt-4 border-t border-slate-100 dark:border-slate-700/50">
            <label className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1"><Volume2 size={12} /> Som do Alerta</label>
            <select value={selectedSound} onChange={(e) => { const s = e.target.value as SoundKey; setSelectedSound(s); previewSound(s); }} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer transition-all">
              {TIMER_SOUNDS.map(s => <option key={s} value={s}>{SOUNDS[s].label}</option>)}
            </select>
          </div>
        </div>
      ) : (
        <div className="py-4 mb-8 italic text-slate-400 dark:text-slate-500 text-sm animate-pulse">{!isRinging ? 'Pausar para ajustar temporizador' : ''}</div>
      )}
      ...
      {isRinging && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/95 backdrop-blur-md animate-in fade-in duration-300 p-4">
          <div className="bg-slate-800 p-10 rounded-[40px] shadow-2xl border border-slate-700 max-w-sm w-full text-center animate-in zoom-in-95 duration-500">
            ...
            <button onClick={stopRinging} className="w-full py-5 bg-red-600 hover:bg-red-500 text-white font-black rounded-2xl text-xl transition-all shadow-xl shadow-red-600/20 active:scale-95 flex items-center justify-center gap-3"><Square size={24} fill="currentColor" /> PARAR</button>
          </div>
        </div>
      )}
    </div>
